        ...
        <div className="lg:col-span-7 flex flex-col space-y-3">
          ...
          <Card className="border border-zinc-200 dark:border-zinc-800 rounded-2xl bg-white dark:bg-zinc-950 overflow-hidden flex-1 flex flex-col min-h-[500px]">
            {selectedEmail ? (() => {
              ...
              const isOfferActive = app && app.waitingListOfferExpiresAt && new Date(app.waitingListOfferExpiresAt) > new Date() && (app.status === 'Pendente' || app.status === 'Em análise');
                      ...
                      isOfferActive && app ? (
                        <div className="flex gap-4 shrink-0">
                          <Button
                            onClick={async () => {
                              try {
                                await acceptWaitlistOffer(app.id);
                                alert('Oferta aceita com sucesso! A vaga foi confirmada.');
                                await loadSimulatedEmails();
                              } catch (err) {
                                console.error(err);
                              }
                            }}
                          ...
                          <Button
                            onClick={async () => {
                              try {
                                await rejectWaitlistOffer(app.id);
